...
import { factory1, factory2, factory3, factory4 } from '../../data/images';
...
export const FactoryGallery: FC<GetToKnowUsProps> = ({ onNavigate }) => {
  const images = [
    {
      src: factory1,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="w-full text-center flex flex-col items-center mb-3 sm:mb-4 lg:mb-5">
          ...
          <h2 className="w-full text-xl sm:text-2xl md:text-3xl lg:text-[32px] xl:text-[34px] font-normal leading-[1.35] tracking-tight">
            <span className="font-semibold text-[#0a1532]">
              With five decades of expertise, Charu Enterprises — India is a global leader in fence fittings manufacturing, delivering 150M+ pieces across four continents. We specialize in Pressed Steel, Malleable & Aluminum Fence Fittings and Tension Bars, galvanized or powder-coated for superior durability.
            </span>{' '}
            <span className="text-slate-500 font-light sm:font-normal">
              A trusted industry name, we have exhibited at FENCETECH for 30+ years and received prestigious Government of India awards. Our 20,000 SQFT factory features an in-house galvanizing plant, die & mold development, and testing lab, enabling high-quality, innovative, and custom solutions for the global market.
            </span>
          </h2>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mb-12 sm:mb-16 lg:mb-20 pt-1 lg:pt-2">
          {images.map((img, idx) => (
            <div
              key={idx}
              className={`group relative rounded-xl sm:rounded-2xl lg:rounded-[26px] overflow-hidden aspect-square w-full shadow-md hover:shadow-xl transition-all duration-500 border border-slate-200/80 bg-slate-100 ${img.isUpper
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 pt-2">
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`flex flex-col items-center justify-center text-center ${index !== 0 ? 'md:border-l md:border-blue-200/70 md:pl-8' : ''
                  }`}
              >
